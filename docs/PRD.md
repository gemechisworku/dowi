...
### D4 — Notifications are best-effort on a PWA *(decision required — see §12)*
...
| Layer | Choice | Why |
|---|---|---|
| Build | Vite 6 + TypeScript (strict) | Fast, tiny output, first-class PWA plugin |
...
| `attachmentIds` | id[] | — | *v1.1* — deferred |
...
- **Currencies & exchange rates (D3, and the resolution of the "any currency" +
  ...
  - *v1.1:* optional one-tap rate refresh when online, off by default.
...
1. Period selector (segmented control: Day / Week / Month / FY) + a period stepper
   ...
   *(forward navigation allowed only if future-dated transactions exist)*.
...
`repeat?` (*v1.1*), `weekKey?` (ISO year-week, set when added to a weekly plan),
...
**Weekly plan & weekly review** *(the habit loop from the brief)*
